...
public class BaseTest3 {
	...
	public void init() {
		// If there is no file then only create
		if (property == null) {

			property = new Properties();
			try {
				FileInputStream fs = new FileInputStream(
						System.getProperty("user.dir") + "\\src\\test\\resources\\projectconfig3.properties");
				property.load(fs);
				// The location of properties file. Path of properties file.
				// System.getProperty("user.dir")+"\\src\\test\\resources\\projectconfig3.properties"
				// ==>> \\src\\test\\resources\\ ==> under this folder create file
				// FILENAME.properties.
				// FileInputStream always should be in try block because any time File not found
				// exception can occur
	...
	public void openBrowser(String bType) {
		...
		if (bType.equalsIgnoreCase("Mozila")) {
			 ...
			 * System.setProperty(FirefoxDriver.SystemProperty.BROWSER_LOGFILE,
			 * System.getProperty("user.dir")+"\\log\\firefox.log"); driver = new
			...
			System.setProperty("webdriver.chrome.driver", property.getProperty("chromeDriver_exe"));
	...
	public WebElement getElement(String locatorKey) {
		...
		/*
		 * Fail the test and report the error. Any time anything can happen Test can
		 * failed for programmer error and element not found on web page or giver
		 * locator wrong for this reason we need to take it inside try catch blook
		 */
		WebElement e = null;
		try {
